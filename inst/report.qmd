...
inputData <- params$inputData %>% as.data.frame()
```
...
# inputData <- readxl::read_xlsx("/Users/limcw/Desktop/Github/door/example_g1e.xlsx")
...
for(i in 1:ncol(inputData)){
  ...
  if(length(count[,1][!is.na(count[,1])]) <= 10){
    ...
    data <- data.frame()
    summ <- c(min(inputData[,i][!is.na(inputData[,i])], na.rm = TRUE),
